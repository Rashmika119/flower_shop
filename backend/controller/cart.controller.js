...
export const getCartItems = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "get all cart Items", data: responseCartItems });
...
export const addCartItems = async (req, res) => {
    try {
        ...
        const isFlowerIn = flowerArray.some((f) =>
            f.flowerId === flower)
...
export const getTotalCountOfCartItems = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "successfully get the total count of cart items" });

    } catch (error) {
        console.log("can not fetch the total cart item count");
        return res.status(500).json({ message: "cannot fetch the total count of cart items" });
...
export const changeCartItemQuantity = async (req, res) => {
    try {
        ...
        if ((flowerCount < quantity) || (quantity < 0)) {
            return res.status(500).json({message:"the flower count is not in valid range"})

        }
        const response = await Cart.findOneAndUpdate({ _id: cartId, "flowers.flowerId": flowerId },{$set:{"flowers.$.quantity":quantity}});
...
export const removeCartItem=async(req,res)=>{
    try {
        ...
        const flower = await Cart.find({ _id: cartId, "flowers.flowerId": flowerId })
        ...
        res.status(200).json({message:"item delete from the cart is successfully."})
